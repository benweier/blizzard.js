 ...
 *
 * @param {String} key Resource Key (achievenemts, collections, encounters, equipment, character-media, mythic-keystone-profile)
 * @param {Object} args
 * @param {String} args.name The character name
 * @param {String} args.realm The realm name slug
 * @return {Promise} A thenable Promises/A+ reference
 */
WoW.prototype.character = function getCharacter(key = 'index', { realm, name, ...args }) {
  const path = key === 'index' ? `/profile/wow/character/${realm}/${name}` : `/profile/wow/character/${realm}/${name}/${key}`
