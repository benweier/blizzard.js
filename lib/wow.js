...
WoW.prototype.character = function getCharacter(keys, { realm, name, ...args } = {}) {
  ...
  return this.blizzard.get(`/wow/character/${realm}/${name}`, merge({}, args, { params }));
};
 ...
 * @return {Promise}              A thenable Promises/A+ reference
 */
WoW.prototype.item = function getItem({ id, set, bonuses, ...args } = {}) {
  const path = set ? `/wow/item/set/${id}` : `/wow/item/${id}`;
  const bl = bonuses ? args.bonuses.toString() : null;

  return this.blizzard.get(path, merge({}, args, { params: { bl } }));
};
 ...
 * @return {Promise}              A thenable Promises/A+ reference
 */
WoW.prototype.pet = function getPet(key, { id, level, breed, quality, ...args } = {}) {
  if (key === 'list') {
    return this.blizzard.get('/wow/pet/', args);
  }

  const params = { level, breedId: breed, qualityId: quality };

  return this.blizzard.get(`/wow/pet/${key}/${id}`, merge({}, args, { params }));
};
...
WoW.prototype.realms = function getRealms({ realms, ...args } = {}) {
  const params = { realms: realms ? realms.toString() : null };

  return this.blizzard.get('/wow/realm/status', merge({}, args, { params }));
};
