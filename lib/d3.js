...
D3.prototype.data = function data (key, args) {
  return this.request(`/d3/data/${key}/${args.id.replace(/^item\//, '')}`, args);
};
...
D3.prototype.era = function era (args) {
  if (!args.id) {
    return this.request('/data/d3/era/', args);
  }

  if (!args.leaderboard) {
    return this.request(`/data/d3/era/${args.id}`, args);
  }

  return this.request(`/data/d3/era/${args.id}/leaderboard/${args.leaderboard}`, args);
};
...
D3.prototype.profile = function profile (args) {
  if (!args.hero) {
    return this.request(`/d3/profile/${args.tag}/`, args);
  }

  return this.request(`/d3/profile//${args.tag}/hero/${args.hero}`, args);
};
...
D3.prototype.season = function profile (args) {
  if (!args.season && !args.leaderboard) {
    return this.request('/data/d3/season/', args);
  }

  if (!args.leaderboard) {
    return this.request(`/data/d3/season/${args.season}`, args);
  }

  return this.request(`/data/d3/season/${args.season}/${args.leaderboard}`, args);
};
