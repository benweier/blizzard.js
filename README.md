...
All User Account requests take `accessToken` as a request parameter. Access tokens are generated by OAuth 2.0 authorization and are valid for 30 days. How you implement OAuth is up to you, although I recommend checking out Blizzard's own [passport-bnet](https://www.npmjs.com/package/passport-bnet) package and reading the [Using OAuth](https://dev.battle.net/docs/read/oauth) article for more details.

The supported origins for the Account API include: `us`, `eu`, `kr`, and `tw`.
...
```javascript
bnet.account.id({accessToken: users_access_token}, callback);
```
...
```javascript
bnet.account.battletag({accessToken: users_access_token}, callback);
```
